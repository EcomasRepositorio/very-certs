...
const CertificateDetails = ({ participantData }: any) => {
  ...
  const corporationImageUrl = participantData.corporation?.[0]?.corporation
    ?.icon
    ? `${API_BASE_URL}${participantData.corporation[0].corporation.icon}`
    : null;

  const instituteImageUrl = participantData.corporation?.[0]?.corporation
    ?.graduate?.[0]?.corporation?.graduate?.[0]?.institute?.image
