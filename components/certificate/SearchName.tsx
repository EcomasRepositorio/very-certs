...
const SearchByName: React.FC = () => {
  ...
  const searchByName = async (event: FormEvent) => {
    event.preventDefault();
    if (!queryValue.trim()) return;

    setLoading(true);
    try {
      // URL actualizada para buscar por nombre
      const url = `${veryURL()}/search/students?search=${encodeURIComponent(
        queryValue.trim()
      )}`;
  ...
  return (
    <div className="">
      <form onSubmit={searchByName} className="w-full ">
                    ...
                    <TableBody>
                      {selectedStudent.modules.map((module) => (
                        <TableRow key={module.id} className="border-b">
                          ...
                          <TableCell className="text-center px-4 py-2 text-neutral-800 dark:text-neutral-200">
                            {module.studentGraduate.corporation.name ||
                              "ECOMAS"}
                          </TableCell>
                          <TableCell className="text-center px-4 py-2 text-neutral-800 dark:text-neutral-200">
                            {module.code}
