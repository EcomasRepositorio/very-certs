...
const PrincipalHome = () => {
  ...
  const texts = [
    <>
      ...
      con las <span className="text-[#009FB2]">herramientas</span> de VeryCerts, líderes en eficiencia.
    </>,
    <>
      Verifique la <span className="text-[#009FB2]">autenticidad</span> de los certificados
      en <span className="text-[#009FB2]">tiempo real</span> con soluciones de confianza como VeryCerts.
    </>,
    <>
      ...
      <span className="text-[#009FB2]">avanzada</span> y simplificada.
    </>,
  ];
    ...
    <section className="relative flex flex-col md:flex-row items-center justify-between bg-transparent min-h-screen px-6 lg:px-20 py-10 sm:ml-0 lg:ml-40 ">
      ...
      >
        <AnimatePresence mode="wait">
          <motion.h1
            key={currentText}
            className="text-gray-800 dark:text-white text-xl md:text-2xl lg:text-4xl font-bold leading-relaxed max-w-md md:max-w-lg lg:max-w-xl "
      ...
      {/* Imagen hero */}
      <motion.div
  className="flex-1 mt-6 sm:mt-4 md:mt-10 flex justify-center sm:mr-0 lg:mr-20"
  initial={{ opacity: 0, x: 50 }}
  animate={{ opacity: 1, x: 0 }}
  transition={{ duration: 1 }}
>
...
{/* bg-gradient-to-r from-blue-50 via-blue-100 to-blue-200 dark:from-gray-800  dark:via-gray-900 dark:to-black */}//colores que podiran quedar bine para una futura actuallizacion
